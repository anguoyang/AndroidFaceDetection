...
public class MainActivity extends Activity {
	...
	protected void onResume() {
		...
		// TODO Auto-generated method stub
		KeyguardManager myKeyguardManager=(KeyguardManager)getApplicationContext().getSystemService(Context.KEYGUARD_SERVICE);
		if (myKeyguardManager.inKeyguardRestrictedInputMode()) {
			resumePending=true;
			Log.d(tAGString, "screenLocked");
			
		} else {mPreview.obtainCamera();
		resumePending=false;
		Log.d(tAGString,"!screenLocked!");
    ...
    //    private static final  boolean DEBUG=true;
		...
		public void obtainCamera(){
			mCamera=Camera.open(CameraInfo.CAMERA_FACING_BACK);//Use the front camera for debugging & testing
		}
		...
		public void surfaceChanged(SurfaceHolder holder, int format, int width,
				int height) {
			// TODO Auto-generated method stub
			Log.d(TAG_STRING, String.format("SurfaceChanged: format=%d,width=%d,height=%d",format,width,height));
			if(mCamera!=null){
				...
				tgtBitmap=Bitmap.createBitmap(optimalSize.width, optimalSize.height, Bitmap.Config.RGB_565);
				mFaceDetector=new FaceDetector(optimalSize.width, optimalSize.height, MAX_FACES);
				int bufSize=optimalSize.width*optimalSize.height*ImageFormat.getBitsPerPixel(parameters.getPreviewFormat());
		...
		public void surfaceCreated(SurfaceHolder holder) {
			// TODO Auto-generated method stub
			Log.d(TAG_STRING, "surfaceCreated called: surface is"+mHolder.getSurface().getClass().getName());
			if (mCamera != null) {
			   try{
		...
		public void onPreviewFrame(byte[] data, Camera camera) {
			...
			YuvImage yuvImage=new YuvImage(data, ImageFormat.NV21, tgtBitmap.getWidth(), tgtBitmap.getHeight(), null);
			
			Rect rect=new Rect(0, 0, tgtBitmap.getWidth(), tgtBitmap.getHeight());
			 ...
			 tgtBitmap=BitmapFactory.decodeStream(new ByteArrayInputStream(outputStream.toByteArray()), rect, bitOptions);
			...
			for(int i=0;i<numFaceDetected;i++){
				...
				try {
					...
					faceCenterF[i] = eyesMP;
						...
						Log.i("Face",
								i + " " + singleFace.confidence() + " "
										+ singleFace.eyesDistance() + " " + "Pose: ("
										+ singleFace.pose(FaceDetector.Face.EULER_X)
										+ ","
										+ singleFace.pose(FaceDetector.Face.EULER_Y)
										+ ","
										+ singleFace.pose(FaceDetector.Face.EULER_Z)
										+ ")" + "Eyes Midpoint: ("
										+ eyesMP.x + "," + eyesMP.y
										+ ")");
		...
		protected void onDraw(Canvas canvas) {
			// TODO Auto-generated method stub
			Log.d(TAG_STRING, "onDraw called: frame size=" + tgtBitmap.getWidth() + ", " +
					 tgtBitmap.getHeight() + ") display size=(" + getWidth() + ", " +
					 getHeight() + ")");
			super.onDraw(canvas);
			if (tgtBitmap != null) {
					...
					for (int i = 0; i < faceCenterF.length; i++) {
						if (faceCenterF[i] != null) {
							...
							// ratio = eyesDistance[i] i* 4.0f / picWidth;
								...
								Rect tmpRect = new Rect((int) (faceCenterF[i].x - eyeDistancef[i] * 2),
